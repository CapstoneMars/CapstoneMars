...
var SupportLine = function() {
   ...
   while (i < tmp.length) { 
      var lineV = {
         label: 'SupportLine' + i ,
         type: 'line',
         data: barData.map(d => { return { x: d.x, y: tmp[i]} })
      }
      lData.push(lineV);
      i++; 
   }
...
function send_d(start , end , interval){
   //2016-02-05
   var sizeofstart = new Date(Number(start.slice(0,4)), Number(start.slice(5,7))-1, Number(start.slice(8,10)));
   var sizeofend = new Date(Number(end.slice(0,4)), Number(end.slice(5,7))-1, Number(end.slice(8,10)));
